...
function isForecastResponse(x: unknown): x is { list: ForecastItem[] } {
  if (!x || typeof x !== "object") return false;
  return Array.isArray((x as { list?: unknown }).list);
}
...
export default function Weather() {
  ...
  useEffect(() => {
    ...
    const fetchWeather = async () => {
      setError(null);
      if (!API_KEY) {
        setError("OpenWeather API key missing.");
        return;
      }
      setLoading(true);
      try {
        ...
        if (!currentRes.ok)
          throw new Error(`Weather fetch failed (${currentRes.status})`);
        if (!forecastRes.ok)
        ...
        if (!isCurrentWeather(currentJson))
          throw new Error("Weather response shape invalid.");
  ...
  return (
    <section className="rounded-2xl bg-white border border-[#E6EFEA] p-5 shadow-sm h-full flex flex-col">
      {/* CARD TITLE — large, bold, unmistakable */}
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2.5">
          <span className="text-xl">⛅</span>
          <h2 className="text-lg font-bold text-[#0B1B18] tracking-tight">
            Weather
          </h2>
        </div>
        {loading && (
      ...
      {currentDay && currentDay.weather?.[0] && (
        <motion.div
          ...
          className="mb-4"
        >
          {/* Main row: icon + temp left, stats right */}
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <p className="text-[36px] font-extrabold tracking-tight text-[#0B1B18] leading-none">
                  {currentDay.main.temp.toFixed(1)}°C
                </p>
                <p className="text-sm font-semibold text-[#1a3a30] mt-1 leading-tight">
                  {currentDay.name}
                </p>
              ...
              {[
                {
                  icon: "💨",
                  label: "Wind",
                  val: `${currentDay.wind.speed.toFixed(1)} km/h`,
                },
                {
                  icon: "🌧",
                  label: "Rain",
                  val: `${currentDay.clouds?.all ?? 0}%`,
                },
                {
                  icon: "💧",
                  label: "Humidity",
                  val: `${currentDay.main.humidity}%`,
                },
                {
                  icon: "📊",
                  label: "H / L",
                  val: `${currentDay.main.temp_max.toFixed(
                    0
                  )}° / ${currentDay.main.temp_min.toFixed(0)}°`,
                },
              ].map(({ icon, label, val }) => (
                <div key={label} className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-medium uppercase tracking-wide text-slate-400 w-16">
                    {label}
                  </span>
                  <span className="text-[13px] font-semibold text-[#0B1B18]">
                    {val}
                  </span>
                </div>
              ))}
          ...
          <div className="flex flex-wrap gap-1.5 pt-3 border-t border-[#F0F7F3]">
            {[
              ...
              <span
                key={chip}
                className="px-2.5 py-1 bg-[#f4f9f6] border border-[#E0EBE6] rounded-full text-[11px] font-semibold text-[#254e3f]"
              >
                {chip}
              </span>
            ))}
            ...
            <div className="flex gap-1.5">
              {forecastData.map((item, index) => {
                ...
                return (
                  ...
                  >
                    {/* DAY NAME — 11px bold */}
                    <span className="text-[11px] font-bold text-[#254e3f]">
                      {dayName}
                    </span>
                    {icon ? (
                      <Image
                    ...
                    {/* TEMP — 14px bold */}
                    <span className="text-[14px] font-bold text-[#0B1B18] leading-none">
                      {item.main.temp.toFixed(0)}°
                    </span>
                    {/* RAIN — 10px, accent color */}
                    {rainPct !== null ? (
                      <span className="text-[10px] font-medium text-[#0FA3B1]">
                        {rainPct}%
                      </span>
                    ) : (
